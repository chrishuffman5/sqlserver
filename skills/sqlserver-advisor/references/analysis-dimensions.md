...
The captures referenced below are the 16 contract tables loaded by `00-load.sql`: `server_info`, `config`, `db_inventory`, `tables`, `columns`, `indexes`, `index_usage`, `missing_indexes`, `index_physical`, `foreign_keys`, `query_stats`, `wait_stats`, `db_files`, `backup_history`, `stats_health`, `identity_columns`.
...
**Captures that feed it.** `tables` (`is_heap`, `has_primary_key`, `has_clustered_columnstore`, `partition_count`, `data_compression_desc`), `columns` (types, sizes, nullability, identity/computed), `indexes` (clustered key, uniqueness), `foreign_keys` (trusted/disabled/cascade actions), `identity_columns` (range consumption), `index_physical` (forwarded records).

**Main smells detected.** Heaps with significant rows (forwarded records, fat NCI RID locators); large tables with no PK; over-wide tables; deprecated/oversized data types; random-GUID clustered keys (page splits); **untrusted or disabled foreign keys** (lost join elimination, hidden integrity violations); unindexed FK columns; risky cascade actions on big tables; **IDENTITY/SEQUENCE ranges nearing exhaustion** (a fully predictable write outage). (Rules `a01`, `a02`, `a03`, `a12`, `a16`.)
...
**Main smells detected.** Unused indexes with write cost; rarely-used wide indexes; exact-duplicate and left-prefix-overlap indexes; disabled indexes; high-impact missing indexes (ranked by improvement measure); tables drowning in near-duplicate suggestions; heavily fragmented large indexes; low page density; heap forwarded records. (Rules `a04`, `a05`, `a06`, `a07`.)
...
**Captures that feed it.** `tables` (`total_space_mb`, `used_space_mb`, `data_space_mb`, `index_space_mb`, `unused_space_mb`, `row_count`, `data_compression_desc`), `db_inventory` (`total_size_mb`, `recovery_model_desc`, `log_reuse_wait_desc`), `db_files` (MAXSIZE caps, growth-disabled files). Trending uses **multiple captures** across `capture/<server>/<captured_at>/` (see `duckdb-analysis.md` section 6).

**Main smells detected.** Very large uncompressed tables; high unused/orphaned space; over-indexed tables; very large unpartitioned tables; **files at/near their MAXSIZE cap or with autogrowth disabled** (imminent hard stop); above-norm growth between captures. (Rules `a08`, `a13`.)
...
**What "good" looks like.** `AUTO_CREATE_STATISTICS` and `AUTO_UPDATE_STATISTICS` are **on** (the safe default), so the optimizer always has cardinality information for predicates; on busy databases `AUTO_UPDATE_STATISTICS_ASYNC` avoids compile-time stalls. Where auto-stats is deliberately off, a **managed stats-maintenance job demonstrably covers every table**. Per statistic: **modification counters sit below the auto-update threshold**, histograms are reasonably fresh, large tables are sampled adequately, and nothing is frozen under `NORECOMPUTE` without a compensating job.

**Captures that feed it.** `db_inventory` (`is_auto_create_stats_on`, `is_auto_update_stats_on`, `is_auto_update_stats_async_on`), `stats_health` (per-statistic `last_updated`, `rows`, `rows_sampled`, `sample_pct`, `modification_counter`, `no_recompute`), `tables` (size context).

**Main smells detected.** Auto-create stats off; auto-update stats off; async update off on a busy/large database; **per-statistic churn past the engine's auto-update threshold (~`SQRT(1000 × rows)`)**; stats not updated in 90+ days despite churn; sub-5% sampling on million-row tables; `NORECOMPUTE` without a managed regime. (Rules `a11`, `a15`.)

**Key context.** Turning auto-stats *off* is **sometimes intentional** (controlled jobs, avoiding mid-day stalls on a huge table) — flag it and confirm a job compensates rather than assuming a mistake. `modification_counter` tracks **leading-column** churn, so skew in non-leading columns undercounts. The remaining live-only concern is the **ascending-key problem** and histogram-level inspection — for that, point the user to the engineering skill's `scripts/06-statistics-info.sql`.
...
**Main smells detected.** Top aggregate CPU consumers; queries expensive per execution; logical-read (I/O) hogs (correlate with missing-index findings in `a06`); large memory grants (spill/over-grant risk); frequent cheap-each queries that are heavy in total (death by a thousand cuts). (Rule `a09`.)
...
**What "good" looks like.** Instance and database settings match the **hardware and workload**: **cost threshold for parallelism** raised off the 1997-era default of 5; **MAXDOP** set for the core/NUMA layout (not 0 on a big box); **`max server memory` capped** below physical RAM with OS headroom (box product); `optimize for ad hoc workloads` on; no legacy/harmful knobs (`priority boost`, `lightweight pooling`). Databases run a **modern compatibility level**, **`PAGE_VERIFY CHECKSUM`**, and OLTP databases use **RCSI** for non-blocking consistent reads. Files are **pre-sized with sane fixed-MB autogrowth**, logs have healthy VLF counts, tempdb has enough equal data files. **Backups exist, are recent, match the recovery model** (log backups under FULL), and use CHECKSUM. Waits are consistent with a healthy config (no dominant `CXPACKET` from a low cost threshold, no `RESOURCE_SEMAPHORE` queueing).

**Captures that feed it.** `config` (`config_name`, `value_in_use`), `server_info` (`host_cpu_count`, `host_physical_memory_mb`, `edition`, `engine_edition`, `sqlserver_start_time`), `db_inventory` (`compatibility_level`, `is_read_committed_snapshot_on`, `page_verify_option_desc`, `recovery_model_desc`, `log_reuse_wait_desc`), `db_files` (growth/VLF/tempdb layout), `backup_history` (cadence), and `wait_stats` (bottleneck class).

**Main smells detected.** Cost threshold left at 5; MAXDOP 0 / mis-set for NUMA; uncapped `max server memory`; ad-hoc workloads optimization off; legacy/risky settings; RCSI off on OLTP; old compatibility level; non-CHECKSUM page verify; percent/tiny/disabled autogrowth; high VLF counts; tempdb file-count mismatch; never/stale full backups; FULL recovery without log backups; blocked log reuse; a dominant wait type or high signal-wait ratio (CPU pressure). (Rules `a10`, `a11`, `a13`, `a14`, `a17`.)
...
|---|---|---|---|
| **Table design** | `a01`, `a02`, `a03`, `a12`, `a16` | `tables`, `columns`, `indexes`, `foreign_keys`, `identity_columns`, `index_physical` | `sqlserver-engineering` |
| **Indexing** | `a04`, `a05`, `a06`, `a07` | `indexes`, `index_usage`, `missing_indexes`, `index_physical` | `sqlserver-engineering` (design); `sqlserver-operations` (maintenance) |
| **Sizing & capacity** | `a08`, `a13` (caps/growth) | `tables`, `db_files` (+ multi-capture trend) | `sqlserver-operations`; `sqlserver-engineering` (compression) |
| **Statistics** | `a11`, `a15` | `db_inventory`, `stats_health` | `sqlserver-operations`; `sqlserver-engineering` (CE) |
| **Query hotspots** | `a09` | `query_stats` | `sqlserver-engineering` (fix); `sqlserver-monitoring` (find) |
| **Configuration** | `a10`, `a11`, `a13`, `a14`, `a17` | `config`, `server_info`, `db_inventory`, `db_files`, `backup_history`, `wait_stats` | `sqlserver-infrastructure`; `sqlserver-operations` (backup); `sqlserver-monitoring` (waits) |
